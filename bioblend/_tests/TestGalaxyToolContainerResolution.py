...
class TestGalaxyContainerResolution(GalaxyTestBase.GalaxyTestBase):
    ...
    def test_resolve(self):
        ...
        toolbox_requirements_only = self.gi.container_resolution.resolve(tool_id="CONVERTER_parquet_to_csv", requirements_only=True)
        assert isinstance(tool, dict)
    #     print(toolbox)
    #     assert False
    ...
    def test_resolve_toolbox(self):
        ...
        for tool in toolbox_by_resolver_type:
            print(tool)
            assert tool['status']['dependency_type'] is None or tool['status']['container_resolver']['resolver_type'] == 'mulled'
        ...
        for tool in toolbox_by_container_type:
            ...
            assert tool['status']['dependency_type'] is None or tool['status']['container_description']['type'] == 'docker'
